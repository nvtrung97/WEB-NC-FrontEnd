...
export const AuthProvider = ({ children }) => {
    ...
    useEffect(() => {
        const storagedDataUser = localStorage.getItem(global.config.localStorageName);
        if (storagedDataUser) {
            setUser(JSON.parse(storagedDataUser));
    ...
    async function signIn(entity) {
        ...
        localStorage.setItem(global.config.localStorageName, JSON.stringify(response.data));
        return response;
    }
    ...
    return (
        <AuthContext.Provider
            value={{ authenticated: Boolean(user), user, signIn, signUp, otp, signOut }}
        >
            {children}
        </AuthContext.Provider>
    );
